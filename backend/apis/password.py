    ...
    def put(self, account):
        ...
        if (candidate_check == 1):
            ...
            if req['password'] == password:
                ...
                c.execute("UPDATE Candidate SET password = ? WHERE email = ?",(req['new_password'], req['email'],))
                conn.commit()
                conn.close()
                return_val = {
                    'ok' : True,
        ...
        # if user is not a candidate, check if they are an employer and match passworord before changing password 
            ...
            if (employer_check == 1):
                ...
                if req['password'] == password:
                    ...
                    c.execute("UPDATE Employer SET password = ? WHERE email = ?",(req['new_password'], req['email'],))
                    conn.commit()
                    conn.close()
                    return_val = {
                        'email' : account,
        ...
        # if user is not a candidate or emplopyer, check if they are a skills backpack admin and match passworord before changing password 
            ...
            if (skillsadmin_check == 1):
                ...
                if req['password'] == password:
                    ...
                    c.execute("UPDATE SkillsBackpackAdmin SET password = ? WHERE email = ?",(req['new_password'], req['email'],))
                    conn.commit()
                    conn.close()
                    return_val = {
                        'email' : account,
        ...
        # if user is not a candidate or emplopyer or skills backpack admin, check if they are a course admin and match passworord before changing password 
            ...
            if (courseadmin_check == 1):
                ...
                if req['password'] == password:
                    ...
                    c.execute("UPDATE CourseAdmin SET password = ? WHERE email = ?",(req['new_password'], req['email'],))
                    conn.commit()
                    conn.close()
                    return_val = {
                        'email' : account,
